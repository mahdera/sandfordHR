...
public class Language {
	...
	public void addLanguage() {
		try {
			String command = "INSERT INTO tbl_language VALUES(0,'"+this.getLanguage()+"')";
	...
	public static void updateLanguage(int id, String language) {
		try {
			String command = "UPDATE tbl_language SET language = '"+language+"' WHERE id = "+id;
			DBConnection.writeToDatabase(command);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public static void deleteLanguage(int id) {
		try {
			String command = "DELETE FROM tbl_language WHERE id = "+id;
			DBConnection.writeToDatabase(command);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
